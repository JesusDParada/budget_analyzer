...
class Parser {
  ...
  void _parseContent({bool run = true}) {
    ...
    document.findAllElements('sheet').forEach((node) {
      if (run) {
        var sheetName = node.getAttribute('name');
        if (sheetName != null) {
          // Optimized for budget analyzer: only load specific sheets
          bool isNeeded = sheetName == 'PRESUPUESTO DE OBRA' || 
                          RegExp(r'^(\d+(\.\d+)+|M-\d+(\.\d+)+)$').hasMatch(sheetName);
          if (isNeeded) {
            _parseTable(node);
          }
        }
    ...
    if (styles != null) {
      ...
      document.findAllElements('border').forEach((node) {
        ...
        for (var elementName in borderElementNamesList) {
          var elements = node.findElements(elementName);
          XmlElement? element = elements.isNotEmpty ? elements.first : null;
          ...
          var colors = element?.findElements('color');
